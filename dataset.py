...
class dataset(object):
    ...
    def prepare_word2vec(self):
        ...
        model.save('data/word2vec_redial')
        ...
        word2index['_split_'] = len(word2index)+4
        json.dump(word2index, open('word2index_redial.json',
    ...
    def padding_w2v(self, sentence, max_length, transformer=True, pad=0, end=2, unk=3):
        ...
        if len(vector) > max_length:
            ...
            return vector+(max_length-len(vector))*[pad], length,\
                concept_mask+(max_length-len(vector)) * \
                [0], dbpedia_mask+(max_length-len(vector))*[self.entity_max]
    ...
    def co_occurance_ext(self, data):
        ...
        f.close()
        json.dump(list(movie_wordset), open('movie_word.json',
                                            'w', encoding='utf-8'), ensure_ascii=False)
        print(len(new_edges))
            ...
            if context_dict['user'] == re_id and len(contexts) > 0:
                ...
                if len(context_dict['movie']) != 0:
                    for movie in context_dict['movie']:
                        ...
                        cases.append({'contexts': deepcopy(contexts), 'response': response,
                                      'entity': deepcopy(entities), 'movie': movie, 'rec': 1})
                else:
                    cases.append({'contexts': deepcopy(contexts), 'response': response,
                                  'entity': deepcopy(entities), 'movie': 0, 'rec': 0})
